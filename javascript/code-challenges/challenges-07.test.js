...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  const compHeight = (a,b) => parseInt(b.height) - parseInt(a.height);
  return starWarsArr.sort(compHeight);
};
...
const howMuchPencil = (str) => {
  ...
  for(let i = 0; i<=str.length; i++){
      result.push(Array.from(str).slice(i).join(''));
  }
  return result;
};
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  const ingred_list = recipe.ingredients;
  const chompFirstWord = (words) => {
      const idx = words.indexOf(" ");
      return words.slice(idx+1);
  }
  const fn2 = (fn) => (x)=>fn(fn(x))
  const chompUnits = fn2(chompFirstWord);
  return ingred_list.map(chompUnits);
};
