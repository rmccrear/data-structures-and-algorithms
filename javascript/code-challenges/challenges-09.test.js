...
const maxInArray = (arr) => {
  // Solution code here...
  return arr.reduce((max, n)=>n>max?n:max, Number.NEGATIVE_INFINITY)
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const isChar = (fam)=>Object.values(fam).includes(character);
  const isCharAndHasChildren = (fam) => isChar(fam) ? fam.children?.length : false;
  for(let family of arr){
      if(isCharAndHasChildren(family))
          return true;
  }
  return false;
};
