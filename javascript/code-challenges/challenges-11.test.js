...
const totalSum = (input) => {
  // Solution code here...
  const r = input.reduce((aac, arr)=>aac.concat(arr), []).reduce((sum, n)=>sum+n, 0);
  return r;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const isDivBy5 = (n) => n%5==0?true:false;
  const isNum = (val) => !isNaN(val) && typeof val === 'number';
  const and  = (fn1, fn2) => (x) => fn1(x) && fn2(x);
  const pow = (n) => Math.pow(2,n);
  return input.map(arr=>arr.filter(and(isNum, isDivBy5))).map((arr)=>arr.map(pow));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const mOrF = (c) => c.gender === 'male' || c.gender==='female';
  return data.filter(mOrF).map((c)=>c.name).join(" and ");
};
...
let findShortest = (data) => {
  // Solution code here...
  const compareHeight=(a,b)=>a===null || parseInt(a.height, 10)<parseInt(b.height, 10);
  return data.reduce((shortest, c)=>compareHeight(shortest, c)?shortest:c, {name: null, height: null})?.name;
};
