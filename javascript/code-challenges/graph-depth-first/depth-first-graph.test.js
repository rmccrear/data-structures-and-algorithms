const Graph = require('./depth-first-graph');
...
const buildSampleGraph = () => {
  ...
  g.addEdge(A, D); g.addEdge(D, A);
  g.addEdge(B, D); g.addEdge(D, B);
  g.addEdge(B, C); g.addEdge(C, B);
  g.addEdge(D, E); g.addEdge(E, D);
  g.addEdge(D, H); g.addEdge(H, D);
  g.addEdge(D, F); g.addEdge(F, D);
  g.addEdge(F, H); g.addEdge(H, F);
  g.addEdge(C, G); g.addEdge(G, C);
  return {
    graph: g,
...
describe('Depth first traversal of graph', () => {
  ...
  test('should traverse one of the outer nodes last', () => {
    ...
    const depthFirstList = [];
    graph.depthFirstTraversal(A, (n) => depthFirstList.push(n));
    expect(['G', 'E', 'H', 'F'].includes(depthFirstList[depthFirstList.length-1])).toBe(true);
  });
});
