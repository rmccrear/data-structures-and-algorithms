...
const allUpperCase = (arr) => {
  // Solution code here...
  const arr2 = [];
  const fn = (val) => val.toUpperCase();
  const push = (val) => arr2.push(val);
  const push_fn = (val)=> push(fn(val))
  //const push = (val) => arr2.push(fn(val));
  arr.forEach(push_fn);
  return arr2;
};
...
const fizzbuzz = (arr) => {
  ...
  const arr2 = []
  arr.forEach((item)=>{
      if     (item%3===0 && item%5 !==0 ) arr2.push("Fizz");
      else if(item%3!==0 && item%5===0) arr2.push("Buzz");
      else if(item%3===0 && item%5===0) arr2.push("Fizz Buzz")
      else arr2.push(item)
  })
  return arr2;
};
